        ...
        /* multibyte options */
            ...
            switch (type) {
                ...
                case TCP_OPT_TS:
                    ...
                    break;
                case TCP_OPT_SACK:
                    LogDebug("SACK option, len %u", olen);
                    if ((olen != 2) &&
...
static int DecodeTCPPacket(Packet *p, uint8_t *pkt, uint16_t len)
{
...
int DecodeTCP(Packet *p, uint8_t *pkt, uint16_t len)
{
    if (unlikely(DecodeTCPPacket(p,pkt,len) < 0)) {
        LogDebug("invalid TCP packet");
        CLEAR_TCP_PACKET(p);
        return DECODE_FAILED;
    }

#ifdef DEBUG
    LogDebug("TCP sp: %" PRIu32 " -> dp: %" PRIu32 " - HLEN: %" PRIu32 " LEN: %" PRIu32 " %s%s%s%s%s",
...
static int TCPGetWscaleTest01(void)
{
    ...
    DecodeTCP(p, raw_tcp, sizeof(raw_tcp));
