...
static int DecodePartialIPV4(Packet* p, uint8_t* partial_packet, uint16_t len)
{
    ...
    if (len < IPV4_HEADER_LEN) {
        LogDebug("DecodePartialIPV4: ICMPV4_IPV4_TRUNC_PKT");
    ...
    if (IPV4_GET_RAW_VER(icmp4_ip4h) != 4) {
        /** Check the embedded version */
        LogDebug("DecodePartialIPV4: ICMPv4 contains Unknown IPV4 version "
    ...
    switch (IPV4_GET_RAW_IPPROTO(icmp4_ip4h)) {
        case IPPROTO_TCP:
            if (len >= IPV4_HEADER_LEN + TCP_HEADER_LEN ) {
                ...
                LogDebug("DecodePartialIPV4: ICMPV4->IPV4->TCP header sport: "
                ...
                p->icmpv4vars.emb_ip4_proto = IPPROTO_TCP;
                LogDebug("DecodePartialIPV4: ICMPV4->IPV4->TCP partial header sport: "
                           "%"PRIu16" dport %"PRIu16"", p->icmpv4vars.emb_sport,
                            p->icmpv4vars.emb_dport);
            } else {
                LogDebug("DecodePartialIPV4: Warning, ICMPV4->IPV4->TCP "
        ...
        case IPPROTO_UDP:
            if (len >= IPV4_HEADER_LEN + UDP_HEADER_LEN ) {
                ...
                LogDebug("DecodePartialIPV4: ICMPV4->IPV4->UDP header sport: "
                           "%"PRIu16" dport %"PRIu16"", p->icmpv4vars.emb_sport,
                            p->icmpv4vars.emb_dport);
            } else {
                LogDebug("DecodePartialIPV4: Warning, ICMPV4->IPV4->UDP "
                           "header Didn't fit in the packet!");
                p->icmpv4vars.emb_sport = 0;
                p->icmpv4vars.emb_dport = 0;
            }
        ...
        case IPPROTO_ICMP:
            if (len >= IPV4_HEADER_LEN + ICMPV4_HEADER_LEN ) {
                ...
                LogDebug("DecodePartialIPV4: ICMPV4->IPV4->ICMP header");
            }
    ...
    LogDebug("ICMPv4 embedding IPV4 %s->%s - PROTO: %" PRIu32 " ID: %" PRIu32 "", s,d,
...
int DecodeICMPV4(Packet *p, uint8_t *pkt, uint32_t len)
{
    ...
    LogDebug("ICMPV4 TYPE %" PRIu32 " CODE %" PRIu32 "", p->icmpv4h->type, p->icmpv4h->code);
