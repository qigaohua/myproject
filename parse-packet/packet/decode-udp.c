...
static int DecodeUDPPacket(Packet *p, uint8_t *pkt, uint16_t len)
{
...
int DecodeUDP(Packet *p, uint8_t *pkt, uint16_t len)
{
    ...
    LogDebug("UDP sp: %" PRIu32 " -> dp: %" PRIu32 " - HLEN: %" PRIu32 " LEN: %" PRIu32 "",
        UDP_GET_SRC_PORT(p), UDP_GET_DST_PORT(p), UDP_HEADER_LEN, p->payload_len);

    if (unlikely(DecodeTeredo(p, p->payload, p->payload_len) == DECODE_OK)) {
    ...
    if (DecodeVXLANEnabledForPort(p->sp, p->dp) &&
            unlikely(DecodeVXLAN(p, p->payload, p->payload_len) == DECODE_OK)) {
