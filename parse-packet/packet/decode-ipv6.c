 ...
 *
 */
static void DecodeIPv4inIPv6(Packet *p, uint8_t *pkt, uint16_t plen)
{
#if 0
    ...
    if (IP_GET_RAW_VER(pkt) == 4) {
        if (pq != NULL) {
            Packet *tp = PacketTunnelPktSetup(p, pkt, plen, DECODE_TUNNEL_IPV4, pq);
            if (tp != NULL) {
                          ...
                          uint16_t prev_hdrextlen)
{
    ...
    LogDebug("IPV6 FH: offset %u, mf %s, nh %u, id %u/%x",
            p->ip6eh.fh_offset,
    ...
    LogDebug("IPV6 FH: frag_hdr_offset %u, data_offset %u, data_len %u",
            p->ip6eh.fh_header_offset, p->ip6eh.fh_data_offset,
            p->ip6eh.fh_data_len);
}

static void
DecodeIPV6ExtHdrs(Packet *p, uint8_t *pkt, uint16_t len)
{
    // SCEnter();
    ...
    while(1)
    {
        if (nh == IPPROTO_NONE) {
            if (plen > 0) {
                ...
                LogWarn("IPV6_DATA_AFTER_NONE_HEADER");
            }
            return;
        }
        ...
        switch(nh)
        {
            case IPPROTO_TCP:
                ...
                DecodeTCP(p, pkt, plen);
                return;

            case IPPROTO_UDP:
                IPV6_SET_L4PROTO(p,nh);
                DecodeUDP(p, pkt, plen);
                return;
...
#endif
            default:
                LogWarn("IPV6_UNKNOWN_NEXT_HEADER");
                IPV6_SET_L4PROTO(p,nh);
                return;
        }
    }
...
static int DecodeIPV6Packet (Packet *p, uint8_t *pkt, uint16_t len)
{
    ...
    if (unlikely(IP_GET_RAW_VER(pkt) != 6)) {
        LogDebug("wrong ip version %d",IP_GET_RAW_VER(pkt));
...
int DecodeIPV6(Packet *p, uint8_t *pkt, uint16_t len)
{
    ...
    ret = DecodeIPV6Packet (p, pkt, len);
    if (unlikely(ret < 0)) {
        CLEAR_IPV6_PACKET(p);
        return DECODE_FAILED;
    }
    ...
    /* debug print */
    char s[46], d[46];
    PrintInetIPv6((const void *)GET_IPV6_SRC_ADDR(p), s, sizeof(s));
    PrintInetIPv6((const void *)GET_IPV6_DST_ADDR(p), d, sizeof(d));
    LogDebug("IPV6 %s->%s - CLASS: %" PRIu32 " FLOW: %" PRIu32 " NH: %" PRIu32 " PLEN: %" PRIu32 " HLIM: %" PRIu32 "", s,d,
    ...
    switch(IPV6_GET_NH(p)) {
        ...
        case IPPROTO_SCTP:
            ...
            DecodeSCTP(p, pkt + IPV6_HEADER_LEN, IPV6_GET_PLEN(p));
            return DECODE_OK;
        case IPPROTO_GRE:
            DecodeGRE(p, pkt + IPV6_HEADER_LEN, IPV6_GET_PLEN(p));
            break;
        case IPPROTO_FRAGMENT:
        case IPPROTO_HOPOPTS:
        case IPPROTO_ROUTING:
        ...
        // case IPPROTO_HIP:
        // case IPPROTO_SHIM6:
            DecodeIPV6ExtHdrs(p, pkt + IPV6_HEADER_LEN, IPV6_GET_PLEN(p));
            break;
        case IPPROTO_ICMP:
            // ENGINE_SET_EVENT(p,IPV6_WITH_ICMPV4);
            LogWarn("IPV6_WITH_ICMPV4");
            break;
#if 0
        case IPPROTO_ICMPV6:
