...
static int DecodeIPV4Options(Packet *p, uint8_t *pkt, uint16_t len, IPV4Options *opts)
{
    ...
    while (plen) {
        ...
        if (*pkt == IPV4_OPT_EOL) {
            ...
            LogDebug("IPV4OPT %" PRIu8 " len 1 @ %d/%d",
                   *pkt, (len - plen), (len - 1));
        ...
        /* multibyte options */
            ...
            switch (opt.type) {
                ...
                case IPV4_OPT_RTRALT:
                    ...
                    break;
                default:
                    LogDebug("IPV4OPT <unknown> (%" PRIu8 ") len %" PRIu8,
                           opt.type, opt.len);
...
static int DecodeIPV4Packet(Packet *p, uint8_t *pkt, uint16_t len)
{
    if (unlikely(len < IPV4_HEADER_LEN)) {
        LogDebug("packet len < ip header len(20)", len);
        return -1;
    }
    ...
    if (unlikely(IPV4_GET_HLEN(p) < IPV4_HEADER_LEN)) {
        LogDebug("ip header too small: %x < 20", IPV4_GET_HLEN(p));
        p->flags |= PKT_IS_INVALID;
        return -1;
    }

    if (unlikely(IPV4_GET_IPLEN(p) < IPV4_GET_HLEN(p))) {
        LogDebug("ip len (%d) <  ip header (%d) ",IPV4_GET_IPLEN(p) ,IPV4_GET_HLEN(p));
        p->flags |= PKT_IS_INVALID;
        return -1;
    }

    // if (unlikely(len < IPV4_GET_IPLEN(p))) {
    //     LogDebug("packet len (%d) < ip len (%d) ", len, IPV4_GET_IPLEN(p));
    //     p->flags |= PKT_IS_INVALID;
    //     return -1;
    // }
...
{
    LogDebug("pkt %p len %"PRIu16"", pkt, len);

    /* do the actual decoding */
    if (unlikely(DecodeIPV4Packet (p, pkt, len) < 0)) {
        printf("decoding IPv4 packet failed");
        CLEAR_IPV4_PACKET((p));
    ...
    LogDebug("IPV4 %s->%s PROTO: %" PRIu32 " OFFSET: %" PRIu32 " RF: %" PRIu32 " DF: %" PRIu32 " MF: %" PRIu32 " ID: %" PRIu32 "", s,d,
    ...
    switch (IPV4_GET_IPPROTO(p)) {
...
#if 0
        case IPPROTO_SCTP:
            DecodeSCTP(p, pkt + IPV4_GET_HLEN(p),
                      IPV4_GET_IPLEN(p) - IPV4_GET_HLEN(p));
            break;
        case IPPROTO_IPV6:
            {
        ...
        case IPPROTO_IP:
            ...
            break;
        case IPPROTO_GRE:
            DecodeGRE(p, pkt + IPV4_GET_HLEN(p),
                      IPV4_GET_IPLEN(p) - IPV4_GET_HLEN(p));
            break;
        case IPPROTO_ICMPV6:
...
#endif
        default:
            LogDebug("p %p pkt %p IPPROTO %04x not supported", p,
                       pkt, IPV4_GET_IPPROTO(p));
    }
...
static int DecodeEthernetTestIPv4Opt(void)
{
    ...
    // };
    uint8_t raw_eth[] = {
        0xae, 0x71, 0x00, 0x00, 0x00, 0x4b, 0x06, 0x90, 0x61, 0x02, 0x00, 0xcd, 0x08, 0x00, 0x41, 0x00,
        0x15, 0x00, 0x80, 0x64, 0x00, 0x21, 0x4c, 0x00, 0x00, 0x30, 0x42, 0xd6, 0xff, 0xff, 0xbd, 0x2f,
        0x02, 0x02, 0x00, 0x02, 0x02, 0x02, 0x02, 0x02, 0x02, 0x02, 0x02, 0x02, 0x02, 0x02, 0x02, 0x02,
        0x02, 0x02, 0x02, 0x02, 0x02, 0x02, 0x02, 0x02, 0x02, 0x02, 0x02, 0x02, 0x02, 0x02, 0x02, 0x02,
        0x01, 0x44, 0x05, 0x22, 0x02, 0x01
    ...
    DecodeEthernet(p, raw_eth, sizeof(raw_eth));
...
int main(int argc, char *argv[])
{
    ...
    // UtRunTests("DecodeIPV4Options");
    // UtRunTests("IPV4Calculate.*ValidChecksum");
    UtRunTests("DecodeEthernet");
    UtCleanup();
    return 0;
}
#endif
