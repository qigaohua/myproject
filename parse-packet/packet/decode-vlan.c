...
static int DecodeIEEE8021ah(Packet *p, uint8_t *pkt, uint16_t len);
...
int DecodeVLAN(Packet *p, uint8_t *pkt, uint32_t len)
{
    ...
    LogDebug("p %p pkt %p VLAN protocol %04x VLAN PRI %d VLAN CFI %d VLAN ID %d Len: %" PRIu32 "",
    ...
    switch (proto)   {
        case ETHERNET_TYPE_IP:
            ...
            DecodeIPV4(p, pkt + VLAN_HEADER_LEN, len - VLAN_HEADER_LEN);
            break;
        case ETHERNET_TYPE_IPV6:
            if (unlikely(len > VLAN_HEADER_LEN + USHRT_MAX)) {
                return DECODE_FAILED;
            }
            DecodeIPV6(p, pkt + VLAN_HEADER_LEN,
                       len - VLAN_HEADER_LEN);
            break;
#if 0
        ...
        case ETHERNET_TYPE_8021AD:
            if (p->vlan_idx >= 2) {
                ...
                DecodeVLAN(p, pkt + VLAN_HEADER_LEN, len - VLAN_HEADER_LEN);
            }
            break;
        case ETHERNET_TYPE_8021AH:
            DecodeIEEE8021ah(p, pkt + VLAN_HEADER_LEN, len - VLAN_HEADER_LEN);
            break;
        case ETHERNET_TYPE_ARP:
            break;
        default:
            LogDebug("unknown VLAN type: %" PRIx32 "", proto);
...
uint16_t DecodeVLANGetId(Packet *p, uint8_t layer)
{
    if (unlikely(layer > 1))
        return 0;
...
static int DecodeIEEE8021ah(Packet *p, uint8_t *pkt, uint16_t len)
{
    ...
    switch (next_proto) {
        ...
        case ETHERNET_TYPE_8021QINQ: {
            DecodeVLAN(p, pkt + IEEE8021AH_HEADER_LEN, len - IEEE8021AH_HEADER_LEN);
            break;
        }
    }
    return DECODE_OK;
